(function () {
    ...
    configuration = {
        ...
        database_directory: '/a/database/Translator/database/data/',
        range: 128,
    ...
    function init(callback) {
        ...
        configuration.properties.forEach(function (property) {
            ...
            configuration.files[property + '_index'][0] = fs.openSync(path.join(configuration.database_directory, property + '_sorted.index'), 'r');
            configuration.files[property + '_index'][1] = fs.fstatSync(configuration.files[property + '_index'][0]).size;
            configuration.files[property + '_data'] = fs.openSync(path.join(configuration.database_directory, property + '.data'), 'r');
        });
        return true;
    }
    ...
    function read_location_word(file, location, range) {
        ...
        var buffer = Buffer.alloc(read_end - read_begain);
