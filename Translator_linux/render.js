...
function init() {
    ...
    electron.ipcRenderer.on('learn_delay', function (event, arg) {
        var delay = +$('#word').val().trim();
        delay && (configuration.learn_demon_word_delay = delay);
        if (is_auto_learn) {
            learn_demon.stop();
            learn_demon.start(configuration.learn_demon_word_delay);
        }
    ...
    electron.ipcRenderer.on('previous_word', function (event, arg) {
        translator.previous_word();
    });
    electron.ipcRenderer.on('next_word', function (event, arg) {
        translator.next_word();
    });
